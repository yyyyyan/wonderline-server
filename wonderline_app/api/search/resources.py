...
from wonderline_app.core.api_logics import handle_request, search_users, search_users_in_trip
...
@search_namespace.route("/trip/<string:tripId>/users")
class SearchUser(Resource):
    @search_namespace.expect(search_users_parser, validate=True)
    @search_namespace.marshal_with(search_users_response_model)
    def get(self, tripId):
